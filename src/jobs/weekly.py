...
def get_weekly_daily_videos(
    ...
    expected_paths = (
        get_expected_weekly_video_paths(
            camera=camera,
            end_date=end_date,
        )
    )
    ...
    return [
        video_path
        for video_path in expected_paths
...
def create_weekly_video(
    ...
    if not available_videos:
        logger.warning(
            "No Daily videos available - "
            f"skipping Weekly for camera: {camera}"
        )

        return None

    temp_directory = (
        get_weekly_temp_directory(
            camera=camera,
            end_date=end_date,
        )
    )
...
def run_weekly_job(
    ...
    logger.info(
        f"Starting Weekly timelapse job "
        f"for rolling window ending {end_date}"
    )

    # Process each camera independently.
    for camera in cameras:
        logger.info(
            f"Processing Weekly for camera: "
            f"{camera}"
        )
