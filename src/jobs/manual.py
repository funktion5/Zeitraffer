...
def run_manual_job(
    ...
    logger.info(
        f"Starting manual timelapse job for {target_date}"
    )
    ...
    if requested_cameras:
        ...
        for requested_camera in dict.fromkeys(
            requested_cameras
        ):
            if requested_camera not in available_cameras:
                logger.error(
                    f"Requested camera not found: "
                    f"{requested_camera}"
                )
                continue
    ...
    for camera in cameras:
        ...
        if existing_video.exists():
            logger.info(
                ...
                f"skipping camera: {existing_video}"
            )
            continue
        ...
        try:
            ...
            if not images:
                logger.warning(
                    "Found 0 images - skipping camera."
                )
            ...
            logger.debug(
                f"First image: {images[0].name}"
            )

            logger.debug(
                f"Last image: {images[-1].name}"
            )
            ...
            logger.exception(
                "Failed to process timelapse "
                f"for camera: {camera}"
            )
            continue

        logger.info(
            f"Finished processing camera: {camera}"
        )
