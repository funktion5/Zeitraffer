...
def run_monthly_job(
	...
	logger.info(
		"Starting Monthly timelapse job for "
		f"{start_date} to {end_date}"
	)

	for camera in cameras:
		logger.info(
			f"Processing Monthly for camera: {camera}"
		)
		...
		try:
			...
			logger.info(
				f"Finished Monthly for camera: {camera}"
			)
			...
			logger.exception(
				"Failed to process Monthly for camera: "
				f"{camera}"
			)

	logger.info(
		"Monthly timelapse job finished for "
		f"{start_date} to {end_date}"
	)